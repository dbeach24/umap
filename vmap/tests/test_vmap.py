...
from vmap.utils import deheap_sort
from vmap.nndescent import (
    make_initialisations,
    make_initialized_nnd_search,
    initialise_search,
)
import vmap.validation as valid
import vmap.sparse as spdist
import vmap.distances as dist
...
"""
Tests for to ensure things are working as expected.
"""
import warnings
...
def test_vmap_trustworthiness_on_iris():
    data = iris.data
    embedding = VMAP(n_neighbors=10, min_dist=0.01, random_state=42).fit_transform(data)
...
def test_supervised_vmap_trustworthiness_on_iris():
    data = iris.data
    embedding = VMAP(n_neighbors=10, min_dist=0.01, random_state=42).fit_transform(
        data, iris.target
    )
    trust = trustworthiness(iris.data, embedding, 10)
    assert_greater_equal(
...
def test_semisupervised_vmap_trustworthiness_on_iris():
    data = iris.data
...
def test_initialized_vmap_trustworthiness_on_iris():
    data = iris.data
    embedding = VMAP(
...
def test_vmap_transform_on_iris_modified_dtype():
    data = iris.data[iris_selection]
    fitter = VMAP(n_neighbors=10, min_dist=0.01, random_state=42).fit(data)
...
# @SkipTest
# def test_vmap_regression_supervision(): # pragma: no cover
